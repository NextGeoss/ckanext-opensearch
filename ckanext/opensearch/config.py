...
def load_settings(settings_name):
    ...
    default_location = "ckanext.opensearch.defaults:{}.toml".format(settings_name)
...
def get_site_url():
    ...
    return six.text_type(
        os.environ.get("CKAN_SITE_URL", config.get("ckan.site_url", ""))
    ).strip()
...
def get_site_title():
    ...
    return six.text_type(
        os.environ.get("CKAN__SITE_TITLE", config.get("ckan.site_title", "CKAN Portal"))
    ).strip()
...
def get_temporal_start_field():
    ...
    return six.text_type(
        os.environ.get(
            "CKANEXT__OPENSEARCH__TEMPORAL_START",
            config.get("ckanext.opensearch.temporal_start", ""),
        )
    ).strip()
...
def get_temporal_end_field():
    ...
    return six.text_type(
        os.environ.get(
            "CKANEXT__OPENSEARCH__TEMPORAL_END",
            config.get("ckanext.opensearch.temporal_end", ""),
        )
    ).strip()
...
def get_short_name():
    ...
    return six.text_type(
        os.environ.get(
            "CKANEXT__OPENSEARCH__SHORT_NAME",
            config.get("ckanext.opensearch.short_name", "CKAN Portal"),
        )
    ).strip()
